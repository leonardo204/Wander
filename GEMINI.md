# Wander - Gemini Code 개발 가이드 (참조용)

이 문서는 `CLAUDE.local.md` 파일의 내용을 기반으로 생성되었으며, Claude 모델이 이 프로젝트의 개발 가이드로 사용했던 문서입니다. 향후 Gemini 모델은 이 문서를 참조하여 프로젝트의 전반적인 구조, 기술 스택, 디자인 시스템 및 개발 컨벤션을 이해하고 작업을 수행합니다.

**참고:** Gemini 모델과의 대화, 응답 및 진행 상황 보고는 가급적 한글로 진행합니다.

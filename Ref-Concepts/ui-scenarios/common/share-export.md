← [인덱스](../index.md)

---

## 8. 공유 및 내보내기

> **v3.1 변경사항**: Context(일상/외출/여행)별 공유 템플릿 차별화

### 8.0 Context별 공유 템플릿

> **v3.1 신규**: 기록 Context에 따라 공유 정보 및 템플릿 자동 최적화

#### 공유 정보 차별화

| 항목 | 🏠 일상 | 🚶 외출 | ✈️ 여행 |
|------|---------|---------|---------|
| 제목 | 날짜 (요일) | 장소명 중심 | 지역명 + 기간 |
| 날짜 | 단일 날짜 | 단일 날짜 | 날짜 범위 |
| 장소 | 장소 흐름 아이콘 | 방문 장소명 | 방문 장소명 + 지도 |
| 통계 | 📸 사진 수 | 📍 장소 수, 🚗 이동거리 | 📍 장소 수, 🚗 이동거리 |
| 태그 | ❌ | ✅ (최대 2개) | ✅ (최대 3개) |
| AI 스토리 | ❌ | 선택적 | ✅ 권장 |

#### Context별 카드 템플릿

**🏠 일상 카드 (Minimal):**
```
┌─────────────────────────────────┐
│                                 │
│  ┌─────────────────────────┐   │
│  │      [대표 사진]         │   │
│  └─────────────────────────┘   │
│                                 │
│  🏠 1월 17일 금요일             │
│  🏢→☕→🏠                       │
│                                 │
│  📸 8장                         │
│                                 │
│                    Wander 🏷️   │
└─────────────────────────────────┘
```

**🚶 외출 카드 (Standard):**
```
┌─────────────────────────────────┐
│                                 │
│  ┌─────────────────────────┐   │
│  │      [대표 사진]         │   │
│  └─────────────────────────┘   │
│                                 │
│  🚶 성수동 맛집 탐방            │
│  2026.01.18                    │
│                                 │
│  📍 4곳 · 🚗 5km               │
│                                 │
│  #맛집탐방 #성수동              │
│                                 │
│                    Wander 🏷️   │
└─────────────────────────────────┘
```

**✈️ 여행 카드 (Full):**
```
┌─────────────────────────────────┐
│                                 │
│  ┌─────────────────────────┐   │
│  │   [지도 + 사진 콜라주]    │   │
│  └─────────────────────────┘   │
│                                 │
│  ✈️ 제주도 3박4일               │
│  2026.01.20 ~ 01.23            │
│                                 │
│  📍 12곳 · 🚗 245km            │
│                                 │
│  #제주여행 #바다 #맛집          │
│                                 │
│  ─────────────────────────     │
│  "에메랄드빛 바다와 함께한      │
│   힐링 여행이었습니다" ✨        │
│                                 │
│                    Wander 🏷️   │
└─────────────────────────────────┘
```

#### Context별 텍스트 공유 형식

**🏠 일상:**
```
🏠 1월 17일 금요일

🏢→☕→🏠
📸 8장의 순간

─────────────────
📍 Wander로 기록했어요
```

**🚶 외출:**
```
🚶 성수동 맛집 탐방
📅 2026.01.18

📍 어니언 → 대림창고갤러리 → 성수연방 → 할머니국수집
🚗 5km 이동

#맛집탐방 #성수동

─────────────────
📍 Wander로 기록했어요
```

**✈️ 여행:**
```
✈️ 제주도 3박 4일 여행
📅 2026.01.20 ~ 01.23

📅 Day 1 (1/20)
📍 제주공항 → 협재해수욕장 → 애월카페거리

📅 Day 2 (1/21)
📍 한라산 등반 → 1100고지 → 서귀포

📅 Day 3 (1/22)
📍 성산일출봉 → 우도 → 만장굴

📅 Day 4 (1/23)
📍 동문시장 → 제주공항

🚗 총 이동거리: 245km
📍 방문 장소: 12곳

#제주여행 #바다 #맛집

─────────────────
📍 Wander로 기록했어요
```

---

### 8.1 공유 플로우

> **v2.0 변경사항**: 프리미엄 체크 제거, 링크 공유는 서버리스로 인해 미지원

```mermaid
flowchart TD
    A[공유하기 버튼 탭] --> B[공유 옵션 시트]

    B --> C{공유 타입 선택}

    C -->|이미지로 공유| D[결과 카드 이미지 생성]
    D --> E[iOS 공유 시트]

    C -->|텍스트로 공유| F[텍스트 클립보드 복사]
    F --> E

    C -->|앱으로 직접 공유| I{앱 선택}
    I -->|카카오톡| J[카카오톡 SDK 호출]
    I -->|인스타그램| K[인스타 스토리 연동]
    I -->|메시지| L[iMessage 연동]

    E --> M{공유 완료?}
    M -->|성공| N[토스트: 공유 완료]
    M -->|취소| O[시트 닫기]
    M -->|실패| P[에러 메시지]
```

> **참고**: 서버리스 앱으로 웹 링크 공유는 미지원. 이미지/텍스트 공유만 가능.

### 8.2 공유 시트 (SCR-014)

```
┌─────────────────────────────────┐
│           공유하기              │
├─────────────────────────────────┤
│                                 │
│  공유 형식 선택                 │
│                                 │
│ ┌───────────────────────┐       │
│ │ 🖼️ 이미지 카드          │       │
│ │ SNS에 바로 올릴 수 있는  │       │
│ │ 예쁜 카드 이미지         │       │
│ └───────────────────────┘       │
│                                 │
│ ┌───────────────────────┐       │
│ │ 📝 텍스트만            │       │
│ │ 타임라인 텍스트 복사    │       │
│ └───────────────────────┘       │
│                                 │
├─────────────────────────────────┤
│  바로 공유                      │
│                                 │
│ ┌─────┬─────┬─────┬─────┐      │
│ │ 💬  │ 📸  │ 💬  │ ··· │      │
│ │카톡 │인스타│메시지│     │      │
│ └─────┴─────┴─────┴─────┘      │
│                                 │
│  [      기타 앱으로 공유     ]  │
│                                 │
└─────────────────────────────────┘
```

### 8.3 공유 텍스트 출처 표기

#### 8.3.1 출처 표기 정책

공유되는 모든 텍스트(이미지 카드, 텍스트 복사, 내보내기)에 앱 출처를 기본 포함합니다.
단, 사용자가 SNS 앱에서 자유롭게 편집/삭제할 수 있도록 **클립보드 복사 형태**로 제공합니다.

```mermaid
flowchart TD
    A[공유 텍스트 생성] --> B[본문 텍스트 생성]
    B --> C[출처 문구 자동 추가]
    C --> D{공유 방식}

    D -->|클립보드 복사| E[전체 텍스트 복사]
    E --> F[사용자가 SNS에서 붙여넣기]
    F --> G[필요시 출처 삭제/수정 가능]

    D -->|이미지 카드| H{출처 위치 선택}
    H -->|이미지 내 삽입| I[워터마크 형태]
    H -->|캡션으로 분리| J[복사 가능한 텍스트]

    D -->|파일 내보내기| K[파일 하단에 출처 포함]
    K --> L[사용자가 파일에서 편집 가능]
```

#### 8.3.2 출처 문구 형식

| 공유 유형 | 출처 문구 | 편집 가능 여부 |
|-----------|----------|---------------|
| 텍스트 공유 | `📍 Wander로 기록했어요` | ✅ SNS에서 삭제 가능 |
| 이미지 카드 (캡션) | `Made with Wander ✨` | ✅ 캡션에서 삭제 가능 |
| 이미지 카드 (워터마크) | 우하단 작은 로고 | ⚠️ 이미지 편집 필요 |
| Markdown 내보내기 | `> 📍 Wander로 기록한 여행` | ✅ 파일에서 삭제 가능 |
| 퀵모드 공유 | `via Wander` | ✅ SNS에서 삭제 가능 |

#### 8.3.3 출처 포함 텍스트 예시

> **v3.1 변경사항**: Context 배지 포함

**✈️ 여행 텍스트 공유 시:**
```
✈️ 제주도 3박 4일 여행

📅 Day 1 (1/15)
📍 제주공항 → 협재해수욕장 → 애월카페거리

📅 Day 2 (1/16)
📍 한라산 등반 → 1100고지 → 서귀포

🚗 총 이동거리: 245km
📍 방문 장소: 12곳

#제주여행 #바다 #맛집

─────────────────
📍 Wander로 기록했어요
```

**🚶 외출 텍스트 공유 시:**
```
🚶 성수동 맛집 탐방
📅 2026.01.18

📍 어니언 → 성수연방 → 할머니국수집
🚗 5km 이동

#맛집탐방 #성수동

─────────────────
📍 Wander로 기록했어요
```

**🏠 일상 텍스트 공유 시:**
```
🏠 1월 17일 금요일

🏢→☕→🏠
📸 8장의 순간

─────────────────
📍 Wander로 기록했어요
```

**퀵모드 "지금 뭐해?" 공유 시:**
```
🎤 홍대 뮤직클럽에서 인디밴드 공연 보는 중!

📍 홍대입구역 근처 · 🕗 저녁 8시

via Wander
```

#### 8.3.4 출처 설정 옵션 (설정 화면)

```
┌─────────────────────────────────┐
│  ◀       공유 설정              │
├─────────────────────────────────┤
│                                 │
│  출처 표기                      │
│ ┌─────────────────────────────┐ │
│ │ 📍 공유 시 출처 포함    🔘  │ │
│ │                             │ │
│ │ 공유하는 텍스트 끝에 앱     │ │
│ │ 출처가 자동으로 추가됩니다.  │ │
│ │ SNS에서 자유롭게 편집/삭제  │ │
│ │ 할 수 있어요.              │ │
│ └─────────────────────────────┘ │
│                                 │
│  출처 문구 스타일               │
│ ┌─────────────────────────────┐ │
│ │ ● 📍 Wander로 기록했어요    │ │
│ │ ○ Made with Wander ✨       │ │
│ │ ○ via Wander               │ │
│ │ ○ #Wander                   │ │
│ └─────────────────────────────┘ │
│                                 │
│  💡 출처를 포함하면 더 많은     │
│     사람들이 Wander를 알게 돼요 │
│                                 │
└─────────────────────────────────┘
```

#### 8.3.5 이미지 카드 출처 옵션

```mermaid
flowchart TD
    A[이미지 카드 생성] --> B{출처 표시 방식}

    B -->|워터마크| C[이미지 우하단 로고]
    C --> D[투명도 조절 가능]
    D --> E[이미지에 고정됨]

    B -->|캡션 분리| F[이미지 + 별도 텍스트]
    F --> G[캡션: Made with Wander]
    G --> H[SNS 게시 시 편집 가능]

    B -->|출처 없음| I[순수 이미지만]
    I --> J[설정에서 비활성화 시]
```

**이미지 카드 옵션 UI 업데이트:**
```
┌─────────────────────────────────┐
│  ◀        카드 스타일     공유  │
├─────────────────────────────────┤
│                                 │
│  ┌─────────────────────────┐   │
│  │                         │   │
│  │    [카드 미리보기]       │   │
│  │                    🏷️   │   │
│  │    🗺️ + 📊         ↑   │   │
│  │              워터마크    │   │
│  └─────────────────────────┘   │
│                                 │
├─────────────────────────────────┤
│  스타일                         │
│  [심플✓] [사진중심] [타임라인]   │
│                                 │
├─────────────────────────────────┤
│  배경색                         │
│  ⚪ ⚫ 🔵 🟢 🟡 🟠 🔴         │
│                                 │
├─────────────────────────────────┤
│  표시 정보                      │
│  ☑️ 지도  ☑️ 날짜  ☑️ 통계      │
│  ☑️ 장소명  ☐ 정확한 주소       │
│                                 │
├─────────────────────────────────┤
│  출처 표기                      │
│  ● 워터마크 (이미지 내)         │
│  ○ 캡션으로 (편집 가능)         │
│  ○ 표시 안 함                   │
│                                 │
└─────────────────────────────────┘
```

#### 8.3.6 SNS별 출처 편집 안내

| SNS | 편집 방법 | 비고 |
|-----|----------|------|
| 카카오톡 | 전송 전 텍스트 직접 편집 | 복사된 텍스트 수정 |
| 인스타그램 스토리 | 텍스트 스티커로 추가됨 → 삭제 가능 | 별도 레이어 |
| 인스타그램 피드 | 캡션에서 편집 | 게시 전 수정 |
| 트위터/X | 트윗 작성 창에서 편집 | 복사된 텍스트 수정 |
| 메시지 | 전송 전 텍스트 편집 | iMessage/SMS |

### 8.4 공유 카드 이미지 옵션

```mermaid
flowchart TD
    A[이미지 카드 선택] --> B[카드 스타일 선택]

    B --> C{스타일 선택}
    C -->|심플| D[지도 + 요약 텍스트]
    C -->|사진 중심| E[사진 그리드 + 정보]
    C -->|타임라인| F[세로형 타임라인]

    D --> G[미리보기 표시]
    E --> G
    F --> G

    G --> H{사용자 액션}
    H -->|배경색 변경| I[색상 선택기]
    H -->|정보 토글| J[표시 정보 선택]
    H -->|공유 진행| K[이미지 생성 및 공유]

    I --> G
    J --> G
```

**카드 스타일 선택 UI:**
```
┌─────────────────────────────────┐
│  ◀        카드 스타일     공유  │
├─────────────────────────────────┤
│                                 │
│  ┌─────────────────────────┐   │
│  │                         │   │
│  │    [카드 미리보기]       │   │
│  │                         │   │
│  │    🗺️ + 📊              │   │
│  │                         │   │
│  └─────────────────────────┘   │
│                                 │
├─────────────────────────────────┤
│  스타일                         │
│  [심플✓] [사진중심] [타임라인]   │
│                                 │
├─────────────────────────────────┤
│  배경색                         │
│  ⚪ ⚫ 🔵 🟢 🟡 🟠 🔴         │
│                                 │
├─────────────────────────────────┤
│  표시 정보                      │
│  ☑️ 지도  ☑️ 날짜  ☑️ 통계      │
│  ☑️ 장소명  ☐ 정확한 주소       │
│                                 │
└─────────────────────────────────┘
```

### 8.5 내보내기 플로우 (SCR-015)

> **v2.1 변경사항**: 이미지(PNG) 내보내기 추가, 워터마크 옵션
> **v2.0 변경사항**: 프리미엄 체크 제거, AI 스토리는 BYOK 연결 여부로 결정

```mermaid
flowchart TD
    A[내보내기 버튼 탭] --> B[내보내기 옵션 시트]

    B --> C{포맷 선택}

    C -->|이미지| D1["PNG 이미지 생성"]
    C -->|Markdown| D[".md 파일 생성"]
    C -->|HTML| E[".html 파일 생성"]
    C -->|텍스트| F[".txt 파일 생성"]
    C -->|JSON| G["데이터 백업용 .json"]

    D1 --> D2[이미지 렌더링]
    D2 --> D3{워터마크}
    D3 -->|포함| D4[우하단 로고 삽입]
    D3 -->|제외| D5[로고 없이 생성]
    D4 --> M
    D5 --> M

    D --> H{내용 옵션}
    E --> H
    F --> H

    H -->|기본| I[구조화된 데이터]
    H -->|AI 스토리 포함| J{BYOK 연결 확인}

    J -->|연결됨| K[AI 스토리 포함 생성]
    J -->|미연결| L[BYOK 연결 안내]

    I --> M[파일 생성]
    K --> M
    G --> M

    M --> N[iOS 공유 시트]
    N --> O{저장 위치}
    O -->|파일 앱| P[파일 앱에 저장]
    O -->|iCloud| Q[iCloud Drive 저장]
    O -->|기타| R[선택한 앱으로 전송]
```

#### 8.5.1 이미지 내보내기 사양

| 항목 | 값 |
|------|-----|
| 형식 | PNG |
| 해상도 | 1080 x 1920 (세로형) |
| 워터마크 | 우하단 Wander 로고 (선택적) |
| 내용 | 지도 + 타임라인 + 통계 요약 |

> **참고**: 프리미엄/크레딧 체크 없음. AI 스토리 포함은 BYOK 연결 여부로만 결정.

### 8.6 내보내기 옵션 시트

> **v2.1 변경사항**: 이미지(PNG) 형식 추가, 워터마크 옵션
> **v2.0 변경사항**: 프리미엄 표시 제거, AI 스토리는 BYOK 연결 시 사용 가능

```
┌─────────────────────────────────┐
│          내보내기               │
├─────────────────────────────────┤
│                                 │
│  파일 형식                      │
│                                 │
│ ┌─────────────────────────────┐ │
│ │ 🖼️ 이미지 (.png)        ⭐  │ │
│ │ SNS 공유에 최적화           │ │
│ └─────────────────────────────┘ │
│                                 │
│ ┌─────────────────────────────┐ │
│ │ 📝 Markdown (.md)           │ │
│ │ 블로그 포스팅에 적합         │ │
│ └─────────────────────────────┘ │
│                                 │
│ ┌─────────────────────────────┐ │
│ │ 🌐 HTML (.html)             │ │
│ │ 웹페이지 형식                │ │
│ └─────────────────────────────┘ │
│                                 │
│ ┌─────────────────────────────┐ │
│ │ 📄 텍스트 (.txt)            │ │
│ │ 순수 텍스트                  │ │
│ └─────────────────────────────┘ │
│                                 │
├─────────────────────────────────┤
│  이미지 옵션 (이미지 선택 시)   │
│                                 │
│  ☑️ Wander 로고 워터마크       │
│                                 │
├─────────────────────────────────┤
│  포함 내용 (문서 형식 선택 시)  │
│                                 │
│  ☑️ 타임라인                    │
│  ☑️ 통계 정보                   │
│  ☐ AI 스토리 (BYOK 연결 필요)  │
│  ☑️ 사진 파일명                 │
│                                 │
├─────────────────────────────────┤
│                                 │
│  [      내보내기 시작      ]    │
│                                 │
└─────────────────────────────────┘
```

> **참고**: AI 스토리는 BYOK 연결 시 무료로 사용 가능.

### 8.7 내보내기 결과물 예시

> **v3.1 변경사항**: Context별 내보내기 형식 차별화

#### ✈️ 여행 Markdown 출력:
```markdown
# ✈️ 제주도 3박 4일 여행

**Context:** 여행
**기간:** 2026.01.15 ~ 2026.01.18
**이동거리:** 245km
**방문장소:** 12곳

---

## Day 1 (2026.01.15)

### 📍 제주공항
- 시간: 10:30
- 유형: ✈️ 공항
- 사진: IMG_001.jpg, IMG_002.jpg

### 📍 협재해수욕장
- 시간: 13:00 ~ 15:30
- 유형: 🏖️ 해변
- 사진: IMG_003.jpg ~ IMG_010.jpg

...

---

> 📍 Wander로 기록한 여행
```

#### 🚶 외출 Markdown 출력:
```markdown
# 🚶 성수동 맛집 탐방

**Context:** 외출
**날짜:** 2026.01.18
**이동거리:** 5km
**방문장소:** 4곳

---

### 📍 어니언
- 시간: 11:00
- 유형: ☕ 카페

### 📍 할머니국수집
- 시간: 12:30
- 유형: 🍜 식당

...

---

> 📍 Wander로 기록했어요
```

#### 🏠 일상 Markdown 출력 (간소화):
```markdown
# 🏠 1월 17일 금요일

**Context:** 일상
**날짜:** 2026.01.17
**사진:** 8장

---

## 장소 흐름
🏢 회사 → ☕ 스타벅스 → 🏠 집

---

> 📍 Wander로 기록했어요
```

#### ✈️ 여행 AI 스토리 포함 Markdown:
```markdown
# ✈️ 바다와 커피향이 함께한 제주 여행

비행기가 제주공항에 내리자마자 느껴지는 바다 냄새.
렌터카를 빌려 곧장 협재해수욕장으로 향했다...

---

## Day 1: 에메랄드빛 첫 만남

### 📍 제주공항
> 설렘을 안고 제주의 땅을 밟았다.
> 공항을 나서자 촉촉한 바다 공기가 반겼다.

...

---

> ✨ Made with Wander
```

---
